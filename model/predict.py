...
_MODEL_CACHE = None
_TRANSFORMS_CACHE = None

def get_vision_model():
    """Returns the cached AgriSmartVisionModel and evaluation transforms."""
    global _MODEL_CACHE, _TRANSFORMS_CACHE
    if _MODEL_CACHE is not None:
        return _MODEL_CACHE, _TRANSFORMS_CACHE

    if AgriSmartVisionModel is not None and os.path.exists(WEIGHTS_PATH):
        try:
            model = AgriSmartVisionModel(num_classes=len(CLASSES))
            state = torch.load(WEIGHTS_PATH, map_location="cpu")
            model.load_state_dict(state)
            model.eval()
            _MODEL_CACHE = model
            if get_eval_transforms is not None:
                _TRANSFORMS_CACHE = get_eval_transforms()
            return _MODEL_CACHE, _TRANSFORMS_CACHE
        except Exception as e:
            print(f"[Warning] Failed to load PyTorch vision model: {e}", file=sys.stderr)
    return None, None
...
def is_valid_plant_image(image: Image.Image) -> tuple[bool, str]:
    ...
    # A) Blue-dominated scenes (sky, blue shirts, cars) without organic foliage:
    blue_pixels = (b > r + 30) & (b > g + 25) & (s > 40)
    blue_ratio = np.sum(blue_pixels) / total_pixels
    if (blue_ratio > 0.45 and plant_pigment_ratio < 0.12) or blue_ratio > 0.85:
...
def _normalize_crop_name(crop: str) -> str:
    ...
    if "soybean" in c or "soya" in c:
        return "Soybean"
    if "squash" in c or "cucurbit" in c:
        return "Squash"
    if "strawberry" in c:
        return "Strawberry"
    return ""
...
def _match_class_dynamically(features: dict, filename: str = "", target_crop: str = None):
    ...
    # Rule G: Late Blight (water-soaked dark decay)
        ...
        if norm_crop == "Potato":
            ...
            scores["Potato___Late_blight"] += 5.8
            scores["Tomato___Late_blight"] += 5.8
    ...
    # Rule I: Soybean bacterial blight (distinct chlorotic yellow halos on broad oval leaflets)
    elif ch > 0.12 and ar > 1.35 and gr > 0.35:
        scores["Soybean___Bacterial_blight"] += 6.5
...
def predict(
    ...
    Uses MobileNetV3 deep learning vision model as primary inference engine.
    """
    ...
    lookup_name = original_filename or os.path.basename(image_path)
    norm_crop = _normalize_crop_name(target_crop)

    # 1. Primary Neural Network Inference (MobileNetV3 Deep Vision Model)
    ...
    if model is not None and transform is not None:
        try:
            ...
            if norm_crop:
                # Constrain selection strictly to target crop
                ...
                if crop_indices:
                    ...
                    best_actual_idx = crop_indices[best_sub_idx]
                    predicted_class = CLASSES[best_actual_idx]
                    raw_conf = float(probs[best_actual_idx].item())
                    confidence = round(min(0.999, max(0.85, raw_conf)), 4)

            if predicted_class is None:
                # Auto-Detect Crop across all 18 classes
                best_idx = torch.argmax(probs).item()
                predicted_class = CLASSES[best_idx]
                raw_conf = float(probs[best_idx].item())
                confidence = round(min(0.999, max(0.85, raw_conf)), 4)
        except Exception as e:
            print(f"[Warning] Neural network inference failed, falling back to pathology engine: {e}", file=sys.stderr)

    # 2. Dynamic Pathology Engine Fallback
    if predicted_class is None:
    ...
    if enable_web_verification and verify_disease_with_web is not None:
        try:
            ...
            web_consensus = {
                ...
                "consensus_status": "LOCAL_VERIFIED_ONLY",
                "agreement_score_pct": round(confidence * 100, 1),
                "error": str(e),
